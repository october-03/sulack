...
const seedChannels = [
  {
    ...
    messages: [
      ...
      { authorEmail: "carol@sulack.local", content: "웹 기본 화면도 준비됐고 이제 인증이랑 채널 목록을 붙이면 될 것 같아요." },
  ...
  },
  {
    ...
    messages: [
      { authorEmail: "bob@sulack.local", content: "인증 검증 구조를 붙인 다음 채널 API와 DM API로 넘어가면 될 것 같습니다." },
      { authorEmail: "dave@sulack.local", content: "로컬 DB에서 마이그레이션이랑 seed 재실행 동작은 제가 같이 확인해볼게요." },
      { authorEmail: "bob@sulack.local", content: "도메인이 아직 작으니 서비스 레이어는 최대한 단순하게 유지하고 싶어요." },
      { authorEmail: "dave@sulack.local", content: "샘플 메시지가 들어가면 읽음 상태 엣지 케이스도 훨씬 보기 쉬워질 것 같아요." },
    ],
  },
  {
    name: "leadership",
...
const seedDirectConversations = [
  ...
  },
  {
    ...
    messages: [
      { authorEmail: "carol@sulack.local", content: "로그인만 동작하면 왼쪽 사이드바를 실제 채널 데이터에 연결할 수 있어요." },
      { authorEmail: "alice@sulack.local", content: "좋아요. unread 상태도 몇 개 남겨두면 목록이 더 살아 보일 것 같아요." },
  ...
  },
  {
    ...
    messages: [
      ...
      { authorEmail: "bob@sulack.local", content: "네. 로컬에서는 기존 상태를 완벽히 보존하는 것보다 재실행 가능성이 더 중요해요." },
...
async function upsertProfiles(client, profileIdsByEmail) {
  for (const user of seedUsers) {
    ...
    if (!userId) {
      throw new Error(`Profile was not created for auth user ${user.email}. Check the DB trigger and Supabase auth settings.`);
    }
